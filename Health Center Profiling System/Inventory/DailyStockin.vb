Public Class DailyStockin
    Private Sub DailyStockin_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadDataDisplay()
    ...
    Sub loadDataDisplay()
        Try
            ...
            sql = "SELECT *,(SELECT  COALESCE(SUM(QuantityS),0)) as stockouttotal, (SELECT BrandName from brand WHERE brand.BrandID=profilemedicine.BrandID) as BrandName,(SELECT UnitName from unit WHERE unit.UnitID=profilemedicine.UnitID) as UnitName,(SELECT CompanyName from company WHERE company.CompanyID=profilemedicine.CompanyID) as CompanyName,
                    (SELECT TypeName from type WHERE type.TypeID=profilemedicine.TypeID) as TypeName From profilemedicine,stockin WHERE stockin.MedicineID=profilemedicine.MedicineID AND stockin.QuantityS <> 0 AND stockin.DOS = '" & Format(CDate(DateTimePicker1.Value).Date, "yyyy-M-d") & "'  GROUP BY MedicineCode DESC"
            ExecuteQuery(sql)
    ...
    Sub loadDataDisplay2()
        Try
            ...
            dbConnect()
            sql = "SELECT *,(SELECT  COALESCE(SUM(QuantityS),0)) as stockouttotal, (SELECT BrandName from brand WHERE brand.BrandID=profilemedicine.BrandID) as BrandName,(SELECT UnitName from unit WHERE unit.UnitID=profilemedicine.UnitID) as UnitName,(SELECT CompanyName from company WHERE company.CompanyID=profilemedicine.CompanyID) as CompanyName,
                    (SELECT TypeName from type WHERE type.TypeID=profilemedicine.TypeID) as TypeName From profilemedicine,stockin WHERE stockin.MedicineID=profilemedicine.MedicineID AND stockin.QuantityS <> 0 AND stockin.MonthS = '" & DateTimePicker2.Value.Month.ToString & "'  GROUP BY MedicineCode DESC"
            ExecuteQuery(sql)
    ...
    Sub loadDataDisplay3()
        Try
            ...
            dbConnect()
            sql = "SELECT *,(SELECT  COALESCE(SUM(QuantityS),0)) as stockouttotal, (SELECT BrandName from brand WHERE brand.BrandID=profilemedicine.BrandID) as BrandName,(SELECT UnitName from unit WHERE unit.UnitID=profilemedicine.UnitID) as UnitName,(SELECT CompanyName from company WHERE company.CompanyID=profilemedicine.CompanyID) as CompanyName,
                    (SELECT TypeName from type WHERE type.TypeID=profilemedicine.TypeID) as TypeName From profilemedicine,stockin WHERE stockin.MedicineID=profilemedicine.MedicineID AND stockin.QuantityS <> 0 AND stockin.YearS = '" & DateTimePicker3.Value.Year.ToString & "'  GROUP BY MedicineCode DESC"
            ExecuteQuery(sql)
    ...
    Private Sub GunaButton1_Click(sender As Object, e As EventArgs) Handles GunaButton1.Click
        Try
            Dim frm As New frmDailyStockin
            Dim rpt As New DailyS
            ...
            With dt
                ...
                .Columns.Add("Type")
                .Columns.Add("Unit")
                .Columns.Add("Total Stockin")
                .Columns.Add("Date")
                .Columns.Add("Grand Total")
            End With

            For Each r As DataGridViewRow In DailyDG.Rows
                dt.Rows.Add(r.Cells(3).Value, r.Cells(2).Value, r.Cells(5).Value, r.Cells(7).Value, r.Cells(4).Value, r.Cells(8).Value, Format(CDate(DateTimePicker1.Value).Date, "yyyy-M-d"), txtremaining.Text)
            Next
    ...
    Private Sub txtsearchstockout_TextChanged(sender As Object, e As EventArgs) Handles txtsearchstockout.TextChanged
        searchstock1(txtsearchstockout.Text, Format(CDate(DateTimePicker1.Value).Date, "yyyy-M-d"))
    End Sub
    Sub searchstock1(tb As String, D2 As Date)
        ...
        Dim i As Integer = 0
        query =
                "SELECT *,(SELECT  COALESCE(SUM(QuantityS),0)) as stockouttotal FROM profilemedicine INNER JOIN stockin ON stockin.MedicineID= profilemedicine.MedicineID INNER JOIN " _
                                ...
                                & " company.CompanyID = profilemedicine.CompanyID INNER JOIN type ON " _
                                 & " type.TypeID=profilemedicine.TypeID INNER JOIN unit ON unit.UnitID=profilemedicine.UnitID" _
                                  & " WHERE DOS = '" & D2 & "' AND QuantityS <> 0 And (MedicineName Like '" & tb & "%') GROUP BY MedicineCode DESC"
    ...
    End Sub
    Sub searchstock2(tb As String, D2 As String)
        ...
        query =
                "SELECT *,(SELECT  COALESCE(SUM(QuantityS),0)) as stockouttotal FROM profilemedicine INNER JOIN stockin ON stockin.MedicineID= profilemedicine.MedicineID INNER JOIN " _
                                ...
                                & " company.CompanyID = profilemedicine.CompanyID INNER JOIN type ON " _
                                 & " type.TypeID=profilemedicine.TypeID INNER JOIN unit ON unit.UnitID=profilemedicine.UnitID" _
                                  & " WHERE MonthS = '" & D2 & "' AND QuantityS <> 0 And (MedicineName Like '" & tb & "%') GROUP BY MedicineCode DESC"
    ...
    Sub searchstock3(tb As String, D2 As String)
        ...
        query =
                "SELECT *,(SELECT  COALESCE(SUM(QuantityS),0)) as stockouttotal FROM profilemedicine INNER JOIN stockin ON stockin.MedicineID= profilemedicine.MedicineID INNER JOIN " _
                                ...
                                & " company.CompanyID = profilemedicine.CompanyID INNER JOIN type ON " _
                                 & " type.TypeID=profilemedicine.TypeID INNER JOIN unit ON unit.UnitID=profilemedicine.UnitID" _
                                  & " WHERE YearS = '" & D2 & "' AND QuantityS <> 0 And (MedicineName Like '" & tb & "%') GROUP BY MedicineCode DESC"
    ...
    Private Sub GunaTextBox1_TextChanged(sender As Object, e As EventArgs) Handles GunaTextBox1.TextChanged
        searchstock2(GunaTextBox1.Text, DateTimePicker2.Value.Month.ToString)
    End Sub
